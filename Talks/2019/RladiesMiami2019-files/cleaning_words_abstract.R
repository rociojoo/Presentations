...
cleaning_words_abstract <- function(papers_topic, path_dict_tools){ 
  ...
  indexRemoveNumbers <- str_detect(df_words$word, "\\-*\\d+\\.*\\d*")
  df_words <- df_words[!indexRemoveNumbers,]
  
  if (class(df_words) == "character"){
    df_words <- data.frame(df_words)
    colnames(df_words) <- "word"
    df_words$word <- as.character(df_words$word)
  }
  ...
  indexRemoveNumbers <- str_detect(df_words$word_am_lem, "\\-*\\d+\\.*\\d*")
  df_words <- df_words[!indexRemoveNumbers,]
  
  indexmore <- which(str_count(df_words$word_am_lem) >1)
  df_words <- df_words[indexmore,]
